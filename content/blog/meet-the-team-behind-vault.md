---
title: "Meet the Team Behind Vault"
slug: "meet-the-team-behind-vault"
date: "2026-03-24"
author: "Vault Team"
excerpt: "Prometheus Labs is building Vault to give everyone access to the yields that used to belong to banks and institutions."
---

![Prometheus Labs — the team building Vault, based in Abu Dhabi under the ADGM regulatory framework](/blog/meet-the-team-behind-vault.svg)

# Meet the Team Behind Vault

Every financial product tells you it's different. We'll let you decide — but first, let us tell you who we are and why we built this.

## Who We Are

Vault is built by **Prometheus Labs**, a fintech company based in Abu Dhabi. We're pursuing regulation under the Abu Dhabi Global Market (ADGM) — one of the world's most respected financial regulatory frameworks.

We're a small team of people who've spent time in banking, financial infrastructure, and consumer technology. We've seen, up close, how yields work at the institutional level. We've watched banks and funds earn 8–10% on capital while offering depositors 0.5%.

That gap bothered us. So we decided to close it.

## Why We Built Vault

The financial system has tiers.

At the top, institutions, funds, and ultra-high-net-worth individuals access lending markets that generate real returns — 4%, 6%, sometimes more. The infrastructure exists. The yields are real. But access has always been restricted by capital minimums, geographic limits, or simply not knowing where to look.

The rest of the world gets savings accounts.

We built Vault because we think that's wrong. Not in a crusading, disrupt-everything way — just in a "this seems like something that can be fixed" way. The technology and infrastructure now exist to route everyday savers into the same types of vetted lending markets that institutions have always used. The margins are different, but the opportunity is real.

Our mission is simple: give everyone access to the yields that used to belong to a privileged few.

## Where We're Based

Abu Dhabi's ADGM framework is one of the reasons we chose to build here. ADGM is an internationally recognized financial center with clear rules, strong oversight, and a genuine commitment to enabling innovative financial products.

Pursuing a credible regulatory licence isn't just a legal requirement — it's a signal. When you're building a product that asks people to trust you with their savings, structure matters. We'll update this when we have our ADGM licence in hand.

## What We Believe

**Transparency over mystique.** We explain how our product works, including the risks. We don't hide behind jargon or obscure the mechanics.

**Access over exclusivity.** The yields we're offering were never unavailable because they were dangerous. They were unavailable because the people who had access to them didn't need to share. We think that should change.

**Simplicity over complexity.** Finance is made to seem complicated. Most of it isn't. We build products that feel like they should — intuitive, honest, and uncomplicated.

**Long-term trust over short-term growth.** We'd rather have a smaller number of users who genuinely understand and trust what we're doing than a large number who signed up without knowing what they got into.

## What's Next

Vault is currently in early access. We're onboarding users carefully — not because we're being coy, but because we want to get the experience right before we scale it.

If you're interested in earning more on your savings with a product built on transparent foundations, we'd love to have you in the early cohort.

[Join the waitlist →](https://vlt.money)

---

*Vault is a product of Prometheus Labs. We are not a bank. Yields are variable and not guaranteed. Please review our terms before depositing.*
